...
export class TableScreenComponent {
  ...
  ngOnInit() {
    this.displayAreaTab = this.commonMiddleWave.FetchAreas().map((area, index) => ({
      title: area.name,                                                                    // Use area name as title
      value: index,                                                                        // Incrementing value from 0 onwards
      content: this.commonMiddleWave.FetchTables().filter(table => table.area === area.id) // Get tables belonging to this area
    }));
